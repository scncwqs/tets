...
[task_local]
0 0 * * * https://raw.githubusercontent.com/passerby-b/JDDJ/main/plantBeans.js

================Loon==============
[Script]
cron "0 0 * * *" script-path=https://raw.githubusercontent.com/passerby-b/JDDJ/main/plantBeans.js,tag=京东到家鲜豆庄园

*/

const $ = new API("plantBeans");
let cookies = [];
...
!(async () => {
    if (cookies.length == 0) {
        if ($.env.isNode) { delete require.cache['./cookie.js']; cookies = require('./cookie.js') }
        else {
            let ckstr = $.read('#cookies');
            if (!!ckstr) {
                if (ckstr.indexOf(',') < 0) {
                    cookies.push(ckstr);
                } else {
